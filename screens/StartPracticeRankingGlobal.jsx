...
export default function StartPracticeRankingGlobal() {
  ...
  const getRanking = async () => {
    ...
    const res = await makeRequest.get("/practice/ranking/global", { headers: { authorization: authState.token }}); 
    return res.data;
  }
  ...
  return (
    <ScrollView style={[styles.mainContainer, {paddingTop: insets.top + 12}]} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.black_01} colors={[COLORS.blue_01]} progressBackgroundColor={COLORS.black_01}/>}>
      ...
      <Text style={styles.title}>Ranking Actual</Text>
      ...
      <View style={styles.box01Container}>
        <View style={styles.box01Header}>
          ...
          :
          <>
            {data?.ownRanking?.avgTime ? <Text style={styles.box01Time}>{parseInt(data.ownRanking.avgTime)}ms</Text> : ""}
            ...
            <Text style={styles.box01Date}>Tops Mejores Promedios</Text>
          </>
          }
        </View>
